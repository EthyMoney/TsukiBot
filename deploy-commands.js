...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    // Every report reply carries window buttons (7d/30d/90d/1y + refresh) and a menu to jump
    // between reports, so the options here only set the starting point. "days", "limit" and
    // "timezone" fall back to the "usage.defaults" section of keys.api when omitted.
    .addSubcommand(s => s.setName('overview').setDescription('Headline numbers: volume, reach, reliability, speed, and the daily trend.')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days (default 30)').setMinValue(1).setMaxValue(3650))
      .addStringOption(o => o.setName('timezone').setDescription('IANA timezone for daily buckets (default from keys.api, else UTC)'))
      .addBooleanOption(o => o.setName('compare').setDescription('Also show the change against the window before this one')))
    .addSubcommand(s => s.setName('commands').setDescription('Most used commands.')
      ...
      .addBooleanOption(o => o.setName('compare').setDescription('Also show the change and rank movement against the window before')))
    ...
    .addSubcommand(s => s.setName('coins').setDescription('Most requested coins.')
      ...
      .addIntegerOption(o => o.setName('limit').setDescription('How many to list (default 15)').setMinValue(1).setMaxValue(50))
      .addBooleanOption(o => o.setName('compare').setDescription('Also show the change against the window before this one')))
    .addSubcommand(s => s.setName('trending').setDescription('Coin momentum: risers, fallers, and coins requested for the first time.')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days, compared with the same window before (default 30)').setMinValue(1).setMaxValue(3650))
      .addIntegerOption(o => o.setName('limit').setDescription('How many movers to list (default 15)').setMinValue(1).setMaxValue(50)))
    .addSubcommand(s => s.setName('activity').setDescription('When the bot gets used: hour, weekday, heatmap.')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days (default 30)').setMinValue(1).setMaxValue(3650))
      .addStringOption(o => o.setName('timezone').setDescription('IANA timezone, e.g. America/Chicago (default from keys.api, else UTC)')))
    ...
    .addSubcommand(s => s.setName('errors').setDescription('What is failing, and what is slow.')
      ...
      .addIntegerOption(o => o.setName('limit').setDescription('How many to list (default 15)').setMinValue(1).setMaxValue(50))
      .addBooleanOption(o => o.setName('compare').setDescription('Also show the change against the window before this one')))
    .addSubcommand(s => s.setName('growth').setDescription('New vs returning users, retention, churn.')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days (default 30)').setMinValue(1).setMaxValue(3650))
      .addStringOption(o => o.setName('timezone').setDescription('IANA timezone for daily buckets (default from keys.api, else UTC)')))
    .addSubcommand(s => s.setName('funnel').setDescription('Do autocomplete searches turn into commands, and what gets searched but never run?')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days (default 30)').setMinValue(1).setMaxValue(3650))
      .addIntegerOption(o => o.setName('limit').setDescription('How many rows to list (default 15)').setMinValue(1).setMaxValue(50)))
    ...
    .addSubcommand(s => s.setName('credits').setDescription('CoinGecko API credit spend, by endpoint, against the monthly quota.')
      .addIntegerOption(o => o.setName('days').setDescription('Window in days (default 30)').setMinValue(1).setMaxValue(3650))
      .addStringOption(o => o.setName('timezone').setDescription('IANA timezone for daily buckets (default from keys.api, else UTC)'))
      .addBooleanOption(o => o.setName('compare').setDescription('Also compare this month with the same point of last month')))
    .addSubcommand(s => s.setName('features').setDescription('What users have set up: price alerts, portfolios, scheduled posts, watchlists.')
      .addIntegerOption(o => o.setName('days').setDescription('Window for the activity figures and the change since (default 30)').setMinValue(1).setMaxValue(3650))
      .addIntegerOption(o => o.setName('limit').setDescription('Coins per list (default 15)').setMinValue(1).setMaxValue(50)))
    .addSubcommand(s => s.setName('storage').setDescription('Table size, row count, and writer health.'))
    .addSubcommand(s => s.setName('dashboard').setDescription('Get a sign-in link for the browser dashboard (runs on the bot host).'))
    .addSubcommand(s => s.setName('digest').setDescription('Build the weekly usage digest now, instead of waiting for the scheduled DM.')
      .addStringOption(o => o.setName('timezone').setDescription('IANA timezone for the daily buckets (default from keys.api, else UTC)')))
    .addSubcommand(s => s.setName('watchdog').setDescription('Run the telemetry watchdog checks now and show what would alert.'))
    .addSubcommand(s => s.setName('prune').setDescription('Permanently delete events older than a cutoff, after a confirmation button.')
      ...
      .addBooleanOption(o => o.setName('confirm').setDescription('Skip the confirmation button and delete immediately'))),
].map(c => c.toJSON());
